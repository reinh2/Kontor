...
func (s *Store) EnsureOperator(ctx context.Context, input CreateOperatorInput) (Operator, error) {
	if s == nil || s.pool == nil { return Operator{}, errors.New("identity: nil store") }
	email, err := NormalizeEmail(input.Email)
	if err != nil { return Operator{}, err }
	operator, err := s.operatorByEmail(ctx, input.TenantID, email)
	if err == nil { return operator, nil }
	if !errors.Is(err, pgx.ErrNoRows) { return Operator{}, err }
	...
	if loaded, loadErr := s.operatorByEmail(ctx, input.TenantID, email); loadErr == nil { return loaded, nil }
